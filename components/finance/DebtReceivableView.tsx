...
import { db, CASHIER_COLLECTION, PURCHASE_ORDERS_COLLECTION, SETTINGS_COLLECTION } from '../../services/firebase';
...
const DebtReceivableView: React.FC<DebtReceivableViewProps> = ({ jobs, userPermissions, showNotification }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
        ...
        try {
            // 1. Fetch POs
            const poSnap = await getDocs(query(collection(db, PURCHASE_ORDERS_COLLECTION), orderBy('createdAt', 'desc')));
            setPurchaseOrders(poSnap.docs.map(d => ({ id: d.id, ...d.data() } as PurchaseOrder)));

            // 2. Fetch Transactions (for balance calculation)
            // Ideally optimize this to fetch only relevant ones or store 'paidAmount' on documents
            const trxSnap = await getDocs(query(collection(db, CASHIER_COLLECTION)));
            setTransactions(trxSnap.docs.map(d => ({ id: d.id, ...d.data() } as CashierTransaction)));

            // 3. Settings for Bank Accounts
            ...
            console.error("Failed loading finance data", e);
        } finally {
            setLoading(false);
        }
    };
  ...
  const receivables = useMemo(() => {
      ...
      // Filter out canceled/deleted.
      // Calculate Paid Amount from Transactions where refJobId matches.
  ...
  const handleSubmitPayment = async (e: React.FormEvent) => {
      ...
      try {
          const payload: CashierTransaction = {
          ...
          // Refresh local state (simple append for immediate feedback, or trigger refetch)
          // For simplicity, we just add to transactions state to trigger re-calc
          setTransactions(prev => [...prev, { ...payload, id: 'TEMP_' + Date.now() }]);
    ...
    <div className="space-y-6 animate-fade-in pb-12">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-6 rounded-xl border border-gray-200 shadow-sm">
            <div>
                ...
                <p className="text-sm text-gray-500 font-medium">Manajemen Tagihan Supplier & Klaim Asuransi/Customer</p>
            </div>
        </div>
