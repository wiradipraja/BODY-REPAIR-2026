...
const SettingsView: React.FC<SettingsViewProps> = ({ currentSettings, refreshSettings, showNotification, userPermissions, realTimeSuppliers }) => {
  ...
  const handleDownloadServiceTemplate = () => {
      ...
      const sampleData = [
          ...
          ['EXT-03', 'Kap Mesin', 'KC', 1.5, 900000],
          ['ENG-01', 'Bongkar Pasang Mesin', 'BP', 0, 1500000]
      ];
      
      const ws = XLSX.utils.aoa_to_sheet([...headers, ...sampleData]);
      const wb = XLSX.utils.book_new();
      XLSX.utils.book_append_sheet(wb, ws, "Template Jasa");
      XLSX.writeFile(wb, "Template_Master_Jasa_Panel.xlsx");
  };
      ...
      <div className="bg-white p-6 rounded-b-xl border border-t-0 border-gray-200 shadow-sm relative min-h-[500px]">
          ...
          {activeTab === 'database' && (
              <div className={`grid grid-cols-1 lg:grid-cols-2 gap-8 ${restrictedClass}`}>
                  ...
                  <div className="lg:col-span-2 bg-gray-50 p-4 rounded-xl border border-gray-200">
                      <div className="flex justify-between items-center mb-3">
                          <h4 className="font-bold text-gray-700 flex items-center gap-2"><CreditCard size={16}/> Rekening Bank Bengkel</h4>
                          <button onClick={() => addItem('workshopBankAccounts', {bankName: '', accountNumber: '', accountHolder: ''})} className="text-xs bg-indigo-100 text-indigo-700 px-2 py-1 rounded font-bold hover:bg-indigo-200"><Plus size={12}/> Tambah</button>
                      </div>
                      <div className="space-y-2">
                          {(localSettings.workshopBankAccounts || []).map((bank, idx) => (
                              <div key={idx} className="flex gap-2 items-center">
                                  <input type="text" placeholder="Nama Bank (BCA, Mandiri)" className="w-1/4 p-2 border rounded text-sm" value={bank.bankName} onChange={e => {
                                      const arr = [...(localSettings.workshopBankAccounts || [])];
                                      arr[idx] = { ...arr[idx], bankName: e.target.value };
                                      setLocalSettings(prev => ({ ...prev, workshopBankAccounts: arr }));
                                  }} />
                                  <input type="text" placeholder="No. Rekening" className="w-1/3 p-2 border rounded text-sm font-mono" value={bank.accountNumber} onChange={e => {
                                      const arr = [...(localSettings.workshopBankAccounts || [])];
                                      arr[idx] = { ...arr[idx], accountNumber: e.target.value };
                                      setLocalSettings(prev => ({ ...prev, workshopBankAccounts: arr }));
                                  }} />
                                  <input type="text" placeholder="Atas Nama" className="flex-grow p-2 border rounded text-sm" value={bank.accountHolder} onChange={e => {
                                      const arr = [...(localSettings.workshopBankAccounts || [])];
                                      arr[idx] = { ...arr[idx], accountHolder: e.target.value };
                                      setLocalSettings(prev => ({ ...prev, workshopBankAccounts: arr }));
                                  }} />
                                  <button onClick={() => removeItem('workshopBankAccounts', idx)} className="text-red-400 hover:text-red-600"><Trash2 size={16}/></button>
                              </div>
                          ))}
                      </div>
                  </div>
          ...
          {activeTab === 'whatsapp' && (
              <div className={`space-y-6 ${restrictedClass}`}>
                  ...
                  <div className="bg-white p-4 rounded-xl border border-gray-200">
                      <h3 className="font-bold text-gray-800 mb-4">Mode Pengiriman</h3>
                      <div className="flex gap-4">
                          <label className="flex items-center gap-2 cursor-pointer p-3 border rounded-lg hover:bg-gray-50 flex-1">
                              <input 
                                  type="radio" 
                                  name="wamode" 
                                  checked={localSettings.whatsappConfig?.mode === 'MANUAL'}
                                  onChange={() => setLocalSettings(prev => ({ ...prev, whatsappConfig: { mode: 'MANUAL' } }))}
                              />
                              <div>
                                  <p className="font-bold text-sm">Manual (Click to Chat)</p>
                                  <p className="text-xs text-gray-500">Membuka aplikasi WhatsApp Web/Desktop saat tombol diklik.</p>
                              </div>
                          </label>
                          <label className="flex items-center gap-2 cursor-pointer p-3 border rounded-lg hover:bg-gray-50 flex-1">
                              <input 
                                  type="radio" 
                                  name="wamode" 
                                  checked={localSettings.whatsappConfig?.mode === 'API'}
                                  onChange={() => setLocalSettings(prev => ({ ...prev, whatsappConfig: { mode: 'API' } }))}
                              />
                              <div>
                                  <p className="font-bold text-sm">Gateway API (Otomatis)</p>
                                  <p className="text-xs text-gray-500">Menggunakan layanan pihak ketiga (WAbot/Twilio) untuk kirim pesan otomatis.</p>
                              </div>
                          </label>
                      </div>
                  </div>
              ...
              <div className={`grid grid-cols-1 lg:grid-cols-3 gap-8 ${restrictedClass}`}>
                          ...
                          <table className="w-full text-sm text-left">
                              <thead className="bg-gray-50 text-gray-600 uppercase sticky top-0">
                                  <tr>
                                      ...
                                      <th className="px-4 py-3 text-center">Nilai Panel</th>
                              ...
                              <tbody className="divide-y divide-gray-100">
                                  {services.map(s => (
                                      <tr key={s.id} className="hover:bg-gray-50">
                                          <td className="px-4 py-2 font-mono text-xs font-bold text-gray-500">{s.serviceCode || '-'}</td>
                                          ...
                                          <td className="px-4 py-2 text-center font-bold">{s.panelValue}</td>
                                          <td className="px-4 py-2 text-right">{new Intl.NumberFormat('id-ID', { style: 'currency', currency: 'IDR', minimumFractionDigits: 0 }).format(s.basePrice)}</td>
                                          <td className="px-4 py-2 flex justify-end gap-2">
                                              <button disabled={!isManager} onClick={() => { setServiceForm(s); setIsEditingService(true); }} className="text-blue-500 hover:text-blue-700"><Edit2 size={16}/></button>
                                              <button disabled={!isManager} onClick={() => handleDeleteService(s.id)} className="text-red-500 hover:text-red-700"><Trash2 size={16}/></button>
                                          </td>
                                      </tr>
                                  ))}
                                  {services.length === 0 && (
                  ...
                  <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 h-fit flex flex-col">
                      ...
                      <form onSubmit={handleSaveService} className="space-y-4">
                          <div>
                              <label className="block text-sm font-medium text-gray-700">Kode Jasa</label>
                              <input disabled={!isManager} type="text" value={serviceForm.serviceCode || ''} onChange={e => setServiceForm({...serviceForm, serviceCode: e.target.value})} className="w-full p-2 border rounded mt-1 font-mono uppercase" placeholder="Contoh: EXT-01"/>
                          </div>
                          <div>
                              <label className="block text-sm font-medium text-gray-700">Nama Pekerjaan *</label>
                              <input disabled={!isManager} required type="text" value={serviceForm.serviceName || ''} onChange={e => setServiceForm({...serviceForm, serviceName: e.target.value})} className="w-full p-2 border rounded mt-1" placeholder="Contoh: Bumper Depan"/>
                          </div>
                          <div>
                              <label className="block text-sm font-medium text-gray-700">Jenis Pekerjaan</label>
                              <select disabled={!isManager} value={serviceForm.workType || 'KC'} onChange={e => setServiceForm({...serviceForm, workType: e.target.value as any})} className="w-full p-2 border rounded mt-1">
                          ...
                          <div className="grid grid-cols-2 gap-3">
                              <div>
                                  <label className="block text-sm font-medium text-gray-700">Nilai Panel</label>
                                  <input disabled={!isManager} type="number" step="0.1" value={serviceForm.panelValue || 0} onChange={e => setServiceForm({...serviceForm, panelValue: Number(e.target.value)})} className="w-full p-2 border rounded mt-1"/>
                              </div>
                              <div>
                                  <label className="block text-sm font-medium text-gray-700">Harga Dasar</label>
                                  <input disabled={!isManager} type="number" value={serviceForm.basePrice || 0} onChange={e => setServiceForm({...serviceForm, basePrice: Number(e.target.value)})} className="w-full p-2 border rounded mt-1"/>
                              </div>
                          </div>
                          <div className="flex gap-2">
                              {isEditingService && <button disabled={!isManager} type="button" onClick={() => { setServiceForm({serviceCode: '', workType: 'KC', panelValue: 1.0}); setIsEditingService(false); }} className="w-1/3 bg-gray-200 text-gray-700 py-2 rounded hover:bg-gray-300 font-medium">Batal</button>}
                              <button disabled={isLoading || !isManager} type="submit" className="flex-grow bg-indigo-600 text-white py-2 rounded hover:bg-indigo-700 font-medium">{isLoading ? 'Menyimpan...' : (isEditingService ? 'Update' : 'Simpan')}</button>
                          </div>
                      </form>
                  </div>
              </div>
